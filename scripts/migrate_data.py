...
def migrate_artifacts(
    conn, s3_client, bucket: str, company_ids: Dict[str, int]
) -> Dict[str, int]:
        ...
        for row in artifacts_csv:
            ...
            if ticker in company_ids:
                try:
                    ...
                    result = cursor.fetchone()
                    if result:
                        artifact_ids[row.get("id", row.get("file_path", ""))] = result[
                            0
                        ]
                except Exception as e:
                ...
                if result:
                    ...
                    """,
                        (
                            ...
                            float(
                                row.get("positive", row.get("positive_score", 0)) or 0
                            ),
                            float(
                                row.get("negative", row.get("negative_score", 0)) or 0
                            ),
                    ...
                    try:
                        values.append(
                            (
                                ...
                                float(
                                    row.get("adj_close", row.get("Adj Close", 0)) or 0
                                ),
                                int(
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate data from S3 to RDS PostgreSQL"
    )
    parser.add_argument(
        "--s3-profile",
        default=os.environ.get("S3_PROFILE", DEFAULT_S3_PROFILE),
        help="AWS profile for S3 access",
    )
    parser.add_argument(
        "--db-profile",
        default=os.environ.get("DB_PROFILE", DEFAULT_DB_PROFILE),
        help="AWS profile for RDS access",
    )
    parser.add_argument(
        "--s3-bucket",
        default=os.environ.get("SOURCE_S3_BUCKET", DEFAULT_S3_BUCKET),
        help="Source S3 bucket name",
    )
    parser.add_argument(
        "--db-host", default=os.environ.get("DB_HOST"), help="RDS endpoint hostname"
    )
    parser.add_argument(
        "--db-name",
        default=os.environ.get("DB_NAME", "cyberrisk"),
        help="Database name",
    )
    parser.add_argument(
        "--db-user",
    ...
    parser.add_argument(
        "--db-password", default=os.environ.get("DB_PASSWORD"), help="Database password"
    )
    parser.add_argument(
        "--region",
    ...
    if not args.db_host:
        print(
            "Error: Database host is required. Use --db-host or set DB_HOST environment variable."
        )
        sys.exit(1)
