...
class ExploriumService:
    ...
    def __init__(self, api_key: Optional[str] = None, cache_ttl: int = 3600):
        ...
        if not self.api_key:
            print(
                "Warning: EXPLORIUM_API_KEY not set. Explorium features will be disabled."
            )
    ...
    def match_business(
        self, company_name: str = None, domain: str = None, ticker: str = None
    ) -> Optional[Dict]:
        """
        ...
        data = {"businesses_to_match": [business_obj]}
    ...
    def fetch_business(
        self, explorium_id: str = None, domain: str = None, ticker: str = None
    ) -> Optional[Dict]:
        """
    ...
    def get_workforce_trends(
        self, explorium_id: str = None, domain: str = None
    ) -> Optional[Dict]:
        """
    ...
    def get_business_events(
        self, explorium_id: str = None, domain: str = None, days_back: int = 90
    ) -> Optional[List[Dict]]:
        """
        ...
        data = {"business_ids": [explorium_id], "event_types": event_types}
        ...
        if business:
            ...
            result["company"] = {
                ...
                "industry": business.get(
                    "linkedin_industry_category", business.get("naics_description")
                ),
                "founded": business.get("year_founded", business.get("founded_year")),
                "headquarters": (
                    headquarters if headquarters else business.get("headquarters")
                ),
            ...
            if emp_range:
                ...
                result["employee_count"] = business.get(
                    "number_of_employees", business.get("employee_count")
                )

            explorium_id = business.get(
                "business_id", business.get("explorium_business_id")
            )
            ...
            if explorium_id:
                ...
                events = self.get_business_events(
                    explorium_id=explorium_id, days_back=90
                )
                if events:
                    ...
                    for event in events:
                        ...
                        if "hiring" in event_name:
                            ...
                            hiring_by_dept[dept] = (
                                hiring_by_dept.get(dept, 0) + job_count
                            )

                    total_jobs = sum(hiring_by_dept.values())
                    result["job_velocity"]["total_postings"] = total_jobs
                    result["job_velocity"]["postings_per_week"] = (
                        round(total_jobs / 13, 1) if total_jobs > 0 else 0
                    )
                    result["job_velocity"]["by_department"] = hiring_by_dept

                    # Determine trend from increase/decrease events
                    increases = len(
                        [e for e in events if "increase" in e.get("event_name", "")]
                    )
                    decreases = len(
                        [e for e in events if "decrease" in e.get("event_name", "")]
                    )
