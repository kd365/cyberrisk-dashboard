...
Tracked cybersecurity companies: CRWD (CrowdStrike), PANW (Palo Alto Networks), ZS (Zscaler), FTNT (Fortinet), OKTA (Okta), NET (Cloudflare), S (SentinelOne), CYBR (CyberArk), TENB (Tenable), RPD (Rapid7)"""
    ...
    def chat(self, message: str, session_id: str,
        ...
        try:
            ...
            final_response = self._process_response(
                response, messages, auth_token
            )
    ...
    def _invoke_claude(self, messages: List[Dict],
                       auth_token: str = None) -> Dict:
        """
    ...
    def _process_response(self, response: Dict, messages: List[Dict],
                          auth_token: str = None,
        ...
        try:
            ...
            if forecast:
                return {
                    ...
                    'confidence': forecast.get('confidence_interval', forecast.get('confidence', {})),
                    'accuracy': forecast.get('accuracy', {}),
                    'computed_at': forecast.get('cached_at', forecast.get('computed_at'))
                }
            ...
            try:
                ...
                try:
                    ...
                    accuracy_metrics = {
                        ...
                        'directional_accuracy': round(eval_results.get('directional_accuracy', 0), 1),
                ...
                logger.warning(f"Chronos forecast failed: {chronos_error}, falling back to Prophet")
