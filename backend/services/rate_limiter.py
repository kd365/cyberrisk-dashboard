...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.user_cooldown_seconds = int(os.environ.get('MODEL_RUN_COOLDOWN', 3600))  # 1 hour
        self.global_daily_limit = int(os.environ.get('MODEL_RUNS_PER_DAY', 100))  # 100 runs/day
    ...
    def check_rate_limit(self, operation: str = 'model_run') -> Tuple[bool, Optional[str]]:
        """
        ...
        if daily_count >= self.global_daily_limit:
            ...
            return False, f"Daily limit reached ({self.global_daily_limit} runs/day). Please use cached results or try again tomorrow."
        ...
        if time_since_last < self.user_cooldown_seconds:
            ...
            return False, f"Please wait {time_str} between model runs. Use cached results in the meantime."
    ...
    def record_run(self, operation: str = 'model_run'):
        ...
        self._daily_runs = {k: v for k, v in self._daily_runs.items() if k >= cutoff_date}

        logger.info(f"Recorded {operation} for user {user_id}. Daily total: {self._daily_runs[today]}")
