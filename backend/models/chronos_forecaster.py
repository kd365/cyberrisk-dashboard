...
class ChronosForecaster:
    ...
    def __init__(self, ticker, model_size="small"):
        ...
        self.model_name = self.MODEL_VARIANTS.get(
            model_size, self.MODEL_VARIANTS["small"]
        )
        self.pipeline = None
        self.data = None
    ...
    def _ensure_model_loaded(self):
        ...
        if not _load_chronos():
            raise RuntimeError(
                "Chronos is not available. Install with: pip install chronos-forecasting torch"
            )
    ...
    def fetch_stock_data(self, period="3y"):
        ...
        # Store raw prices
        df = pd.DataFrame({"ds": hist.index, "price": hist["Close"]})
        ...
        print(
            f"   Log return stats: mean={df['log_return'].mean():.4f}, std={df['log_return'].std():.4f}"
        )
    ...
    def forecast(self, days_ahead=30, num_samples=100):
        ...
        while len(forecast_dates) < days_ahead:
            last_forecast_date = (
                forecast_dates[-1] if len(forecast_dates) > 0 else last_date
            )
        ...
        price_samples_final = self.last_price * np.exp(
            np.cumsum(samples, axis=1)[:, -1]
        )
        ...
        print(
            f"   Confidence Range (P10-P90): ${predicted_prices_p10[-1]:.2f} - ${predicted_prices_p90[-1]:.2f}"
        )
